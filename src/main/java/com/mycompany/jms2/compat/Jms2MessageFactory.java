...
import static com.mycompany.jms2.compat.Jms2Util.execute;
...
public class Jms2MessageFactory {
    ...
    static boolean isBodyAssignableTo(Message message, Class type) {
        ...
        if (TextMessage.class.isInstance(message) && String.class.equals(type)) {
            return true;
        }
    ...
    static <T> T getBody(final Message message, final Class<T> type) {
        return Jms2Util.execute(new Callback<T>() {

            @Override
            public T execute() throws JMSException {
                return doGetBody(message, type);
            }
        });

    }

    private static <T> T doGetBody(Message message, Class<T> type) throws JMSException {
        if (TextMessage.class.isInstance(message)
        ...
        if (BytesMessage.class.isInstance(message) && Objects.equals(byte[].class, type)) {

            BytesMessage bmsg;
            if (Jms2BytesMessage.class.isInstance(message)) {
                bmsg = (BytesMessage) Jms2BytesMessage.class.cast(message).getDelegate();
            } else {
                bmsg = BytesMessage.class.cast(message);
            }
